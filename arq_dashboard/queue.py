...
async def get_job_ids(redis: ArqRedis, queue_name: str) -> Set[str]:
    high_job_ids = set(await redis.zrangebyscore('high', "-inf", "inf"))
    medium_job_ids = set(await redis.zrangebyscore('medium', "-inf", "inf"))
    low_job_ids = set(await redis.zrangebyscore('low', "-inf", "inf"))

    high_job_ids |= medium_job_ids
    high_job_ids |= low_job_ids
    high_job_ids |= await get_result_job_ids(redis)
    return {job_id.decode() for job_id in high_job_ids}
