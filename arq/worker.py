        ...
        if self.allow_pick_jobs:
            async with self.sem:  # don't bother with zrangebyscore until we have "space" to run the jobs
                ...
                job_ids = await self.pool.zrangebyscore(
                    "high", min=float('-inf'), start=self._queue_read_offset, num=count, max=now
                )
                if job_ids == []:
                    job_ids.extend(await self.pool.zrangebyscore(
                        "medium", min=float('-inf'), start=self._queue_read_offset, num=count, max=now))
                if job_ids == []:
                    job_ids.extend(await self.pool.zrangebyscore(
                        "low", min=float('-inf'), start=self._queue_read_offset, num=count, max=now))
    ...
    async def start_jobs(self, job_ids: List[bytes]) -> None:
        ...
        for job_id_b in job_ids:
            ...
            async with self.pool.pipeline(transaction=True) as pipe:
                ...
                priority = 'high'
                score = await pipe.zscore('high', job_id)
                if not score:
                    priority = 'medium'
                    score = await pipe.zscore('medium', job_id)
                    if not score:
                        priority = 'low'
                        score = await pipe.zscore('low', job_id)
                ...
                try:
                    ...
                    t = self.loop.create_task(self.run_job(job_id, int(score), priority))
                    t.add_done_callback(lambda _: self.sem.release())
                    self.tasks[job_id] = t

    async def run_job(self, job_id: str, score: int, priority: str = "high") -> None:  # noqa: C901
        ...
        async with self.pool.pipeline(transaction=True) as pipe:
            ...
            pipe.psetex(  # type: ignore[no-untyped-call]
                priority + job_id, int(self.in_progress_timeout_s * 1000), b'1')  # type: ignore[unused-coroutine]
            if self.allow_abort_jobs:
                pipe.zrem(abort_jobs_ss, job_id)  # type: ignore[unused-coroutine]
                v, job_try, _, ddd, abort_job = await pipe.execute()
            else:
                v, job_try, _, ddd = await pipe.execute()
                abort_job = False
        ...
        if job_try > max_tries:
            ...
            result_queue_name = ''
            async with self.pool.pipeline(transaction=True) as pipe:
                await pipe.exists('low' + job_id)
                await pipe.exists('medium' + job_id)
                await pipe.exists('high' + job_id)

                low, mmedium, hhigh = await pipe.execute()

                if hhigh:
                    result_queue_name = 'high'
                else:
                    if mmedium:
                        result_queue_name = 'medium'
                    else:
                        result_queue_name = 'low'
    ...
    async def finish_job(
        ...
        async with self.pool.pipeline(transaction=True) as tr:
            ...
            if finish:
                ...
                delete_keys += [retry_key_prefix + job_id, job_key_prefix + job_id, 'low' + job_id, 'medium' + job_id, 'high' + job_id,]
                tr.zrem(abort_jobs_ss, job_id)  # type: ignore[unused-coroutine]
                tr.zrem('high', job_id)  # type: ignore[unused-coroutine]
                tr.zrem('medium', job_id)  # type: ignore[unused-coroutine]
                tr.zrem('low', job_id)  # type: ignore[unused-coroutine]
            elif incr_score:
                tr.zincrby('high', incr_score, job_id)  # type: ignore[unused-coroutine]
                tr.zincrby('medium', incr_score, job_id)  # type: ignore[unused-coroutine]
                tr.zincrby('low', incr_score, job_id)  # type: ignore[unused-coroutine]
            if delete_keys:
