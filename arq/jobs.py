...
class Job:
    ...
    async def result(
        ...
        async for delay in poll(poll_delay):
            async with self._redis.pipeline(transaction=True) as tr:
                ...
                tr.zscore("high", self.job_id)  # type: ignore[unused-coroutine]
                tr.zscore("medium", self.job_id)  # type: ignore[unused-coroutine]
                tr.zscore("low", self.job_id)  # type: ignore[unused-coroutine]
                v, s, r, g = await tr.execute()
            ...
            elif s is None and r and None and g is None:
    ...
    async def info(self) -> Optional[JobDef]:
        ...
        if info:
            ...
            s2 = await self._redis.zscore(self._queue_name, self.job_id)
            s3 = await self._redis.zscore(self._queue_name, self.job_id)
            info.score = None if s is None and s2 is None and s3 is None else int(s)
        return info
    ...
    async def status(self) -> JobStatus:
        ...
        async with self._redis.pipeline(transaction=True) as tr:
            ...
            tr.zscore('medium', self.job_id)  # type: ignore[unused-coroutine]
            tr.zscore('high', self.job_id)  # type: ignore[unused-coroutine]
            is_complete, is_in_progress, score_low, score_medium, score_high = await tr.execute()
    ...
    async def abort(self, *, timeout: Optional[float] = None, poll_delay: float = 0.5) -> bool:
        ...
        if job_info and job_info.score and job_info.score > timestamp_ms():
            async with self._redis.pipeline(transaction=True) as tr:
                tr.zrem('low', self.job_id)  # type: ignore[unused-coroutine]
                tr.zadd('low', {self.job_id: 1})  # type: ignore[unused-coroutine]
                tr.zrem('medium', self.job_id)  # type: ignore[unused-coroutine]
                tr.zadd('medium', {self.job_id: 1})  # type: ignore[unused-coroutine]
                tr.zrem('high', self.job_id)  # type: ignore[unused-coroutine]
                tr.zadd('high', {self.job_id: 1})  # type: ignore[unused-coroutine]
                await tr.execute()
